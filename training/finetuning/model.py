...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        H, W = self.input_resolution
        B, L, C = x.shape
        assert L == H * W, "Input feature length does not match the resolution."
        assert C == self.in_dim, f"Input channel count {C} does not match expected in_dim {self.in_dim}."
        assert H % 2 == 0 and W % 2 == 0, f"Input resolution ({H}*{W}) is not even."
...
class Core(nn.Module):
    ...
    def forward(self, x):
        ...
        B, L, C = x.shape
        H, W = self.input_resolution
        assert C == self.dim and L == H * W, f"Invalid Core input shape: expected (B, {self.dim}, {H*W}), got {x.shape}."
